...
public class UserDaoTest {
	...
	private List<User> loadUsers() throws IOException {
		...
		var temp = Files.lines(Paths.get("../aws5.rtf")).map(line -> line.split("[^A-Za-z]")).map(Arrays::asList)
				.flatMap(list -> list.stream()).filter(word -> word.length() > 3 && word.length() < 20)
				.map(word -> new User(word, "pass" + word)).limit(testusersno).collect(Collectors.toList());
	...
	public void setUp() throws SQLException, IOException {
		...
		conn = SwingDatabase.ins().getconnection();
	...
	public void testSave() throws SQLException { // Testing the save method of my UserDao class
		UserDao ud = new UserDao();
		User user = new User("Javascript", "C++");
		// ud.save(user);
		var stmt = conn.createStatement();
		...
		assertEquals("User password does not match retrieved", user.getPassword(), password);
		stmt.close();
	}
